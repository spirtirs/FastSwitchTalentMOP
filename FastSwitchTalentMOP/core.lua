...
macro:SetScript('OnLeave', function(self)
    self:Hide()
    GameTooltip_Hide()
    if self:GetParent() and self:GetParent().SetScript then
        self:GetParent():SetScript('OnLeave', GameTooltip_Hide)
        -- Восстанавливаем оригинальные свойства фрейма
        if self:GetParent().talentButton then
            self:GetParent().talentButton:Show()
        end
    end
end) 
hooksecurefunc('PlayerTalentFrameTalent_OnEnter', function(self)
    ...
    if not selected and not available then
        -- Сохраняем ссылку на оригинальный фрейм
        macro.talentButton = self
...
local f = CreateFrame("Frame", "FastSwitchTalentMOPFrame")
f:SetScript("OnEvent", function(self, event, ...)
    if event == "PLAYER_LOGIN" or event == "PLAYER_ENTERING_WORLD" or event == "SPELLS_CHANGED" or event == "ACTIONBAR_SLOT_CHANGED" then
